...
class Homologa:
    ...
    def get_homologa_all(self, usrdep, inicio, final):
        if inicio == "00-00-0000" and final == "00-00-0000":
            ...
            if usrdep != 0 :
                ...
                s = s + " where idEstado in (4, 5, 82, 83) and TipoCircun = 1 and DEP = %d and Convert(CHAR(10),fechaAct,23) between %d and %d order by prov, sec"
                self.cur.execute(s, lista)
            else:
                lista = inicio, final
                s = s + " where idEstado in (4, 5, 82, 83) and TipoCircun = 1 and Convert(CHAR(10),fechaAct,23) between %d and %d order by Dep, Prov, Sec"
                self.cur.execute(s, lista)
    ...
    def get_homologa_idloc(self, idlocreci):   
        ...
        if usrdep != 0 :
            ...
            lista = inicio, final
            s = s + " where TipoCircun = 1 and Convert(CHAR(10),fechaAct,23) between %d and %d order by Dep, Prov, Sec"
            self.cur.execute(s, lista)
