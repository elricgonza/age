...
def recinto(idreci, idlocreci):
        ...
        if idreci == '0':  # es NEW
            if False:   # valida si neces POST
                ...
                rc.add_recinto(idlocreci[1], nextid, request.form['nomreci'], request.form['zonareci'], \
                               ...
                               request.form['aulasreci'], request.form['pisosreci'], \
    ...
    # New
    return render_template('recinto.html', error=error, rc=rc, load=False, puede_editar=p, titulo='Registro de Zonas y Distritos')


@app.route('/get_asientos_all', methods=['GET', 'POST'])
def get_asientos_all():
    rca = recia.Reciasiento(cxms)
    rows = rca.get_asientos_all(usrdep)
    if rows:
        return jsonify(rows)
    else:
        return jsonify(departamento='COORDENADA',
...
@app.route('/get_zonas_all', methods=['GET', 'POST'])
def get_zonas_all():
    rca = recia.Reciasiento(cxms)
    rows = rca.get_zonas_all(usrdep)
    if rows:
        return jsonify(rows)
    else:
        return jsonify(departamento='COORDENADA',
...
@app.route('/zonas/<idloc>/<idzona>/<iddist>', methods=['GET', 'POST'])
@login_required
def zonas(idloc, idzona, iddist):
    z = zo.Zonas(cxms)
    ...
    if request.method == 'POST':
        ...
        if idzona == '0' and iddist == '0':  # es NEW
            if False:   # valida si neces POST
                #error = "El usuario: " + request.form['uname']  + " ya existe...!"
                #return render_template('asiento.html', error=error, u=u, load_u=True)
                print('msg-err')
                ...
                z.add_zona(request.form['idloc'], nextidzona, request.form['nomzona'], \
                           nextiddist, request.form['fechaIngreso'][:-7], fa, request.form['usuario'])
            ...
            fa = str(datetime.datetime.now())[:-7]
            z.upd_zona(request.form['idloc'], idzona, request.form['nomzona'], fa, request.form['usuario'])
        ...
        if idzona != '0' and iddist != '0':  # EDIT
            if z.get_zonadist_idloc(idloc, idzona, iddist) == True:
                """if a.docAct == None:
                    a.docAct = """
                ...
                return render_template('zona.html', error=error, z=z, load=True, puede_editar=p, titulo='Modificación de Zonas y Distritos')

    # New
    return render_template('zona.html', error=error, z=z, load=False, puede_editar=p, titulo='Registro de Zonas y Distritos')


@app.route('/zonasr/<idloc>/<idzona>/<iddist>', methods=['GET', 'POST'])
@login_required
def zonasr(idloc, idzona, iddist):
    z = zo.Zonas(cxms)
    rc = reci.Recintos(cxms)
    ...
    if request.method == 'POST':
        ...
        if idzona == '0' and iddist == '0':  # es NEW
            if False:   # valida si neces POST
                ...
                nextidzona = z.get_next_zona(request.form['idlocreci'])
                nextiddist = z.get_next_dist(request.form['idlocreci'])
                z.add_zona(request.form['idlocreci'], nextidzona, request.form['nomzona'], \
                           nextiddist, request.form['fechaIngreso'][:-7], fa, request.form['usuario'])

                z.add_dist(request.form['idlocreci'], nextiddist, request.form['nrodist'], request.form['nomdist'], \
                           request.form['fechaIngreso'][:-7], fa, request.form['usuario'])
