        ...
        if idreci == '0':  # es NEW
            if False:   # valida si neces POST
                ...
                rc.add_recinto(idlocreci[1], nextid, request.form['nomreci'], request.form['zonareci'], \
                               ...
                               request.form['ruereci'], request.form['edireci'], request.form['depenreci'], \
                               request.form['pisosreci'], \
            ...
            rc.upd_recinto(idlocreci[1], idreci, request.form['nomreci'], request.form['zonareci'], \
                               ...
                               request.form['ruereci'], request.form['edireci'], request.form['depenreci'], \
                               request.form['pisosreci'], \
                               request.form['fechaIngreso'], fa, request.form['usuario'])
        ...
        if idreci != '0':  # EDIT
            if rc.get_recinto_idreci(idreci, idlocreci) == True:
                ...
                return render_template('recinto.html', error=error, rc=rc, load=True, puede_editar=p, asientoRecis=rca.get_asientos_all(usrdep), zonasRecis=rca.get_zonas_all(usrdep),
                                       estados=rc.get_estados(), trecintos=rc.get_tiporecintos())
        ...
        if iddist == '0':  # es NEW
            if False:   # valida si neces POST
                ...
                z.add_dist(request.form['idloc'], nextiddist, request.form['nrodist'], request.form['nomdist'], \
                           request.form['fechaIngreso'][:-7], fa, request.form['usuario'])
                if ban == '1':
                    return ('Distrito Guardado !!! CORRECTAMENTE ¡¡¡')
                else:
                    rows = z.get_zonas_all(usrdep)
                    return render_template('zonas_list.html', zonas=rows, puede_adicionar='Zonas - Adición' in permisos_usr)  # render a template
        else: # Es Edit
            fa = str(datetime.datetime.now())[:-7]
            z.upd_dist(request.form['idloc'], iddist, request.form['nrodist'], request.form['nomdist'], fa, request.form['usuario'])
...
def zonasr():
    ...
    z.add_zona(request.form['idloc'], nextidzona, request.form['nomzona'], \
               nextiddist, request.form['fingreso'][:-7], fa, request.form['usuario'])
    nomdist = z.get_nomdist(request.form['idloc'], request.form['nomdist'])
    z.add_dist(request.form['idloc'], nextiddist, request.form['circundist'], nomdist, \
               request.form['fingreso'][:-7], fa, request.form['usuario'])
    ...
    if request.method == 'POST':
        ...
        if dep_id == '0':  # es NEW
            if False:   # valida si neces POST
                ...
                s.add_depto(nextid, request.form['nomDep'], request.form['diputados'], request.form['diputadosUninominales'], request.form['pais'],
            ...
            s.upd_depto(dep_id, request.form['nomDep'], request.form['diputados'], request.form['diputadosUninominales'], request.form['pais'], fa, usr)
            rows = s.get_deptos_all(usrdep)
        ...
        if dep_id != '0':  # EDIT
            if s.get_depto_iddep(dep_id) == True:
                ...
                if s.usuario == None:
                    s.usuario = usr
                return render_template('deptosABC.html', error=error, s=s, load=True, titulo='Edicion de Datos de Departamentos', tpaises=s.get_combo_paises(usrdep), puede_editar=p)
    # New
    return render_template('deptosABC.html', error=error, s=s, load=False, titulo='Registro de Nuevo Departamento', tpaises=s.get_combo_paises(usrdep) ,puede_editar=p)
    ...
    if request.method == 'POST':
        ...
        if dep_id == '0' and prov_id == '0':  # es NEW
            if False:   # valida si neces POST
                ...
                s.add_prov(request.form['depto'], nextid, request.form['NomProv'], request.form['codprov'],
                           str(request.form['fechaIngreso'])[:-8],
                           fa, usr)
            ...
            s.upd_prov(dep_id, prov_id, request.form['NomProv'], request.form['codprov'], fa, usr)
            rows = s.get_provs_all(usrdep)
            return render_template('provs_list.html', listaProvincias=rows, puede_editar='Provincias - Edicion' in permisos_usr)  # render a template
    else: # Viene de <asientos_list>
        if dep_id != '0' and prov_id != '0':  # EDIT
            if s.get_provs_id(dep_id, prov_id) == True:
                ...
                return render_template('provsABC.html', error=error, s=s, load=True, titulo='Edicion de Datos de Provincias', tpaises=s.get_combo_paises(usrdep), tdeptos=s.get_combo_deptos(usrdep), puede_editar=p)
    # New
    return render_template('provsABC.html', error=error, s=s, load=False, titulo='Registro de Nueva Provincia', tpaises=s.get_combo_paises(usrdep), tdeptos=s.get_combo_deptos(usrdep) ,puede_editar=p)
...
def munABC(dep_id, prov_id, mun_id):
    ...
    if request.method == 'POST':
        ...
        if dep_id == '0' and prov_id == '0' and mun_id == '0':  # es NEW
            ...
            s.add_mun(request.form['depto'], request.form['prov'], nextid, request.form['numConceSec'], request.form['nomSec'],
            str(request.form['fechaIngreso'])[:-8], fa, usr)

            rows = s.get_mun_all(usrdep)
            return render_template('mun_list.html', municipios=rows, puede_adicionar='Municipios - Adición' in permisos_usr)  # render a template
        else: # Es Edit
            s.upd_mun(dep_id, prov_id, mun_id, request.form['numConceSec'], request.form['nomSec'],
                    fa, usr)
        ...
        if dep_id != '0' and prov_id != '0' and mun_id != '0': # EDIT
            if s.get_mun_id(dep_id, prov_id, mun_id) == True:
                ...
                return render_template('munABC.html', error=error, s=s, load=True, titulo='Edicion de Datos de Municipios', tpaises=s.get_combo_paises(usrdep), tdeptos=s.get_combo_deptos(usrdep), tprovs=s.get_combo_prov(usrdep), puede_editar=p)

    # New
    return render_template('munABC.html', error=error, s=s, load=False, titulo='Registro de Nuevo Municipio', tpaises=s.get_combo_paises(usrdep), tdeptos=s.get_combo_deptos(usrdep), tprovs=s.get_combo_prov(usrdep), puede_adicionar=p)
